...
struct MediaUseCaseTest: MediaUseCaseProtocol {
    func fetchMedia(for type: MovieDB.FetchType, locale: MovieDB.Locale) async throws -> [Media] {
        ...
        case .randomMovies, .cinemaPlaying, .cinemaUpcomimg, .movieTrending, .movieNew: Constants.ResourceFiles.movies
        case .tvTrending, .tvNew: Constants.ResourceFiles.tvShows
        case .personTrending, .personPopular: Constants.ResourceFiles.people
        default: Constants.ResourceFiles.all
        }
    ...
    func fetchMedia(for type: MovieDB.FetchType, locale: MovieDB.Locale, searchText: String) async throws -> [Media] {
        let repository: MovieDBRepositoryProtocol = MovieDBRepositoryTest(resourceName: Constants.ResourceFiles.all)
        return try await repository.fetchMedia().filterWithImage()
    }
}
